package za.co.tman.logging.service.messaging.googlepubsub.impl;
...
import za.co.tman.logging.config.MessageImplementationCondition;
import za.co.tman.logging.config.PubSubMessagingProperties;
...
import za.co.tman.logging.service.messaging.googlepubsub.GoogleChannelManager;
...
public class GoogleChannelManagerImpl implements GoogleChannelManager {
    ...
    public PubSubInboundChannelAdapter messageChannelAdapterGenericSub(
        ...
        try {
            return getPubSubInboundChannelAdapter(inputChannel, "LoggingGenericSub");
        } catch (Exception ex) {
            ex.printStackTrace();
            System.exit(-1);
            return null;
    ...
    public PubSubInboundChannelAdapter messageChannelAdapterIncidentSub(
        ...
        try {
            return getPubSubInboundChannelAdapter(inputChannel, "IncidentTopicSub");
        } catch (Exception ex) {
            ex.printStackTrace();
            System.exit(-1);
            return null;
    ...
    /**
     * Retrieve an channel adapter for in pubSubInputChannel and incidentTopic
     *
     * @param inputChannel
     * @param incidentTopicSub
     * @return
     */
    private PubSubInboundChannelAdapter getPubSubInboundChannelAdapter(@Qualifier("pubsubInputChannel") MessageChannel inputChannel,
                                                                       String incidentTopicSub) {
     ...
     * Each module has a list of topics that a pubSubMessageType must be submitted to.
     *
